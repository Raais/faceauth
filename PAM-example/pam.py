...
def doAuth(pamh):
    ...
    if status == 3:
        if not config.getboolean("core", "suppress_unknown"):
            pamh.conversation(pamh.Message(
                pamh.PAM_ERROR_MSG, "No face model known"))

        syslog.syslog(syslog.LOG_NOTICE, "Failure, no face model known")
        syslog.closelog()
        return pamh.PAM_USER_UNKNOWN

    # Status 2 means we exceded the timeout period
    elif status == 2:
    ...
    # Status 4 means no camera was detected
    elif status == 4:
        syslog.syslog(syslog.LOG_INFO, "Failure, no camera detected")
        syslog.closelog()
        pamh.conversation(pamh.Message(pamh.PAM_ERROR_MSG,
                                       "No camera detected"))
        return pamh.PAM_AUTH_ERR

    # Status 1 is a successful exit
    elif status == 1:
